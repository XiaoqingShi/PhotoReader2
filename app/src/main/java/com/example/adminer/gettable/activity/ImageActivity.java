    ...
    public void makeTagForRequest(final int photoId)
    {
        ...
        new Thread() {
            ...
            public void run() {
                ...
                String path = "http://125.217.32.3:8080/PhotoRecognition/addMarkRecord.action?u_id=1&p_id=" + photoId + "&tagName=" + tagName[0];
                try {
        ...
        new Thread() {
            ...
            public void run() {
                ...
                String path = "http://125.217.32.3:8080/PhotoRecognition/addMarkRecord.action?u_id=1&p_id=" + photoId + "&tagName=" + tagName[1];
                try {
        ...
        new Thread() {
            @Override
            public void run() {
                String path = "http://125.217.32.3:8080/PhotoRecognition/addMarkRecord.action?u_id=1&p_id=" + photoId + "&tagName=" + tagName[2];
                try {
                    URL url = new URL(path);
                    HttpURLConnection conn = (HttpURLConnection) url.openConnection();
                    conn.setRequestMethod("GET");
